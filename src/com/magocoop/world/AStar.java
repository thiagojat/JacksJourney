		...
		public static List<Node> findPath(World world, Vector2i start, Vector2i end){
			...
			while(openList.size() > 0) {
				...
				for(int i = 0; i < 9; i++) {
					...
					int xi = (i%3) - 1;
					int yi = (i/3) - 1;
					Tile tile = World.tiles[x+xi+((y+yi)*World.WIDHT)];
					if(tile == null) continue;
					if(tile instanceof WallTile2 || tile instanceof GridTile) continue;
					if(i == 0) {
						Tile test = World.tiles[x+xi+1+((y+yi) * World.WIDHT)];
						Tile test2 = World.tiles[x+xi+((y+yi+1) * World.WIDHT)];
							if(test instanceof WallTile2 || test2 instanceof WallTile2 
							|| test instanceof GridTile || test2 instanceof GridTile) {
								continue;
							}
					}
					else if(i == 2) {
						...
						Tile test = World.tiles[x+xi+((y+yi-1) * World.WIDHT)];
						Tile test2 = World.tiles[x+xi+1+((y+yi) * World.WIDHT)];
							if(test instanceof WallTile2 || test2 instanceof WallTile2 
							|| test instanceof GridTile || test2 instanceof GridTile) {
								continue;
							}
